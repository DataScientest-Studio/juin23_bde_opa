...
[,python]
----
include::../opa/financial_data.py[lines=8..15]
----

=== Access to providers' data

An abstract class is defined to retrieve data from a provider, with 2 simple methods to get either streaming or historical data.

[#python-stock-market-provider-definition]
.Definition of StockMarketProvider abstract class
[,python]
----
include::../opa/providers/abstract.py[lines=7..17]
----
...
An link:../opa/app_secrets.py[app_secrets.py] module has been added to handle those secrets. It relies either on the existence of a `./secrets` directory at the root of the projet (that should be created on the local clone of the repo) or a `/secrets` directory created by Docker Secrets.
